...
bool operator> (Fraction& A, Fraction& B) {
	...
	if (A.sign == 1 && B.sign == 1) {
		...
		for (int posA = (A.size - 1), int posB = (B.size - 1), int i = 0; i < A.getIntSize(); posA--, posB--, i++) {
			if (A[posA] > B[posB]) return true;
			if (B[posB] > A[posA]) return false;
		};
		for (int posA = A.decimals - 1, int posB = B.decimals - 1, int i = 0; i < min(A.decimals, B.decimals); posA--, posB--, i++) {
			if (A[posA] > B[posB]) return true;
			if (B[posB] > A[posA]) return false;
		}
...
Fraction& operator- (Fraction& A, Fraction& B) {
	Fraction* result = new Fraction(max(A.size, B.size));
	for (int i = 0; i < MAXSIZE; i++)
		(*result)[i] = A[i] - B[i];
	return *result;
}